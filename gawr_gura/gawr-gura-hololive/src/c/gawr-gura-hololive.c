...
static BitmapLayer *s_gura_layer;
static GBitmap *s_gura_bitmap;
...
static void main_window_load(Window *window) {
  ...
  // bitmaps
  // bg
  s_gura_layer = bitmap_layer_create(GRect((bounds.size.w - 180) / 2, (bounds.size.h - 180) / 2, 180, 180));
  // bitmap_layer_set_compositing_mode(s_gura_layer, GCompOpSet);
  s_gura_bitmap = gbitmap_create_with_resource(RESOURCE_ID_IMAGE_GURA);
  bitmap_layer_set_bitmap(s_gura_layer, s_gura_bitmap);

  // stack up the layers
  layer_add_child(window_layer, bitmap_layer_get_layer(s_gura_layer));
...
static void main_window_unload(Window *window) {
  ...
  bitmap_layer_destroy(s_gura_layer);
  
  // unload gbitmaps
  gbitmap_destroy(s_gura_bitmap);
}
